(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('raitamitraApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-raitamitraApp-error');
                var entityKey = httpResponse.headers('X-raitamitraApp-params');
                if (errorHeader) {
